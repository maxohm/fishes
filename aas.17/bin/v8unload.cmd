...
if %xNULPRM4%==.nul (
	...
	if not exist %xTSPRM4%\%xTIMESTAMP%.dt exit /B 6
        if exist %xTSPRM4%\%xTIMESTAMP%.dt del %xTSPRM4%\%xTIMESTAMP%.dt
	set xRESULT=%xTSPRM4%\%xTIMESTAMP%.dt
	
)

:stage5
set xPARAMS=CONFIG /N"v8service" /P"v8service" /F%xOBJECT% /DisableUpMessages /Out%xLOGFILE% -NoTruncate /DumpIB%xRESULT%
